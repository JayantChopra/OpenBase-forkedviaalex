...
def assess_scalability(codebase_path: str) -> Tuple[float, List[str]]:
    ...
    Returns a tuple of (score, details) where score is a float between 2.0 and 10.0
    and details is a list of human-readable findings.
    """
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    flags = _detect_import_features(
                        name_lower,
                        threading_keywords,
                        caching_keywords,
                        web_framework_keywords,
                        database_keywords,
                        queue_keywords,
                        pooling_keywords,
                        config_keywords,
                    )
                    uses_asyncio = uses_asyncio or flags["asyncio"]
                    uses_multiprocessing = uses_multiprocessing or flags["multiprocessing"]
                    uses_threading = uses_threading or flags["threading"]
                    uses_caching_libs = uses_caching_libs or flags["caching"]
                    uses_web_frameworks = uses_web_frameworks or flags["web"]
                    uses_database_libs = uses_database_libs or flags["db"]
                    uses_queue_systems = uses_queue_systems or flags["queue"]
                    uses_connection_pooling = uses_connection_pooling or flags["pooling"]
                    config_management = config_management or flags["config"]
...
def _detect_import_features(
    module_name: str,
    threading_keywords: List[str],
    caching_keywords: List[str],
    web_framework_keywords: List[str],
    database_keywords: List[str],
    queue_keywords: List[str],
    pooling_keywords: List[str],
    config_keywords: List[str],
) -> Dict[str, bool]:
    """Return a dict of feature flags detected from a module/import name."""
    name_lower = module_name.lower()
    return {
        "asyncio": "asyncio" in name_lower,
        "multiprocessing": "multiprocessing" in name_lower,
        "threading": any(kw in name_lower for kw in threading_keywords),
        "caching": any(kw in name_lower for kw in caching_keywords),
        "web": any(kw in name_lower for kw in web_framework_keywords),
        "db": any(kw in name_lower for kw in database_keywords),
        "queue": any(kw in name_lower for kw in queue_keywords),
        "pooling": any(kw in name_lower for kw in pooling_keywords),
        "config": any(kw in name_lower for kw in config_keywords),
    }
...
def _analyze_dependencies(python_files, codebase_path: str) -> float:
    ...
    for file_path in python_files:
        try:
            ...
            logger.warning("Failed to process imports in %s: %s", file_path, e)
            continue
    ...
    def _is_stdlib_import(imp: str) -> bool:
        for lib in ('os', 'sys', 'json', 'time', 'datetime', 'collections', 'itertools'):
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    abstract_methods = _count_abstract_methods(node)
                    if abstract_methods > 0:
                        interfaces += 1

                    # Check for singleton pattern
                    if _has_singleton_pattern(node):
                        singletons += 1

                    # Check for factory pattern
                    if _has_factory_pattern(node):
                        factories += 1
            ...
            logger.warning("Failed to analyze design patterns in %s: %s", file_path, e)
            continue
...
def _analyze_data_flow(python_files) -> float:
    ...
    for file_path in python_files:
        try:
            ...
            logger.warning("Failed to analyze data flow in %s: %s", file_path, e)
            continue
...
# Helper internal utilities for design pattern analysis
def _is_abc_base(base) -> bool:
    """Return True if AST base indicates ABC usage."""
    return isinstance(base, ast.Name) and base.id == 'ABC'


def _count_abstract_methods(class_node: ast.ClassDef) -> int:
    """Count methods decorated as abstractmethod within a class AST node."""
    count = 0
...
def _has_singleton_pattern(class_node: ast.ClassDef) -> bool:
    """Heuristic detection of singleton pattern in a class AST node."""
...
def _has_factory_pattern(class_node: ast.ClassDef) -> bool:
    """Heuristic detection of factory pattern in class methods or name."""
